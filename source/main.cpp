...
void InitializeData() {
    ...
    for (int i = 1; i < n1; ++i) {
        for (int j = 1; j < n2; ++j) {
            for (int k = 1; k < n3; ++k) {
                u1Con->elem(i, j, k) = u1nCon->elem(i, j, k) = u10;
                u2Con->elem(i, j, k) = u2nCon->elem(i, j, k) = u20;
                u3Con->elem(i, j, k) = u3nCon->elem(i, j, k) = u30;
                roCon->elem(i, j, k) = ronCon->elem(i, j, k) = ro0_g;
                tCon->elem(i, j, k) = tnCon->elem(i, j, k) = t0;
            }
        }
    }
